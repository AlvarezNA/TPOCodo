...
class Producto:
    ...
    def __init__(self, codigo, descripcion, cantidad, precio, imagen):
        ...
        self.imagen = imagen           # imagen del producto, grabar archivo "codigo + prod.jpg"
    ...
    def modificar(self, nueva_descripcion, nueva_cantidad, nuevo_precio, nueva_imagen):
        ...
        self.imagen = nueva_imagen            # Modifica imagen del producto, grabar archivo "codigo + prod.jpg"
...
class Inventario:
    # Definimos el constructor e inicializamos los atributos de instancia
    def __init__(self):
        # me conecto a la DB y creo cursor para poder operar sobre la misma
        self.conexion = get_db_connection()
        self.cursor = self.conexion.cursor()

    #agrego productos en la BD inventario
    def agregar_producto(self, codigo, descripcion, cantidad, precio, imagen):
        ...
        nuevo_producto = Producto(codigo, descripcion, cantidad, precio, imagen)
    ...
    #consulto si un prodcuto existe o no en la BD
    ...
    def modificar_producto(self, codigo, nueva_descripcion, nueva_cantidad, nuevo_precio, nueva_imagen):
        ...
        if producto:
            ...
            sql = f'UPDATE productos SET descripcion = "{nueva_descripcion}", cantidad = {nueva_cantidad}, precio = {nuevo_precio}, imagen = "{nueva_imagen}" WHERE codigo = {codigo};' 
            self.cursor.execute(sql)
            self.conexion.commit()
...
class Carrito:
    ...
    #muestro contenido del carrito
    def mostrar(self):
        productos_carrito = []
        for item in self.items:
            producto = {'codigo': item.codigo, 'descripcion': item.descripcion, 'cantidad': item.cantidad, 'precio': item.precio, 'imagen': item.imagen}
